 ...
 ** Date:		10/21/2019
 ** Description: Contains definitions for dynamic array functions
*****************************************************************************/
...
{	
	...
	TYPE * oldRef = v->data; /*memorize old*/
	int oldArrSize = v->size;

	/*allocate doubled memory*/
	initDynArr(v, 2 * v->capacity);

	for (int i = 0; i < oldArrSize; i++) /*copy old*/
	{	
		v->data[i] = oldRef[i];
	}
	v->size = oldArrSize;

	free(oldRef); /*free old memory*/
}
